...
fn find_node_by_path_transparent<'a>(
...
fn find_node_by_path_mut_transparent<'a>(
    ...
    match parse_document(&content) {
        Ok((_remaining, mut nodes)) => {
            ...
            if changed_fields.is_empty() {
                ...
                let has_template_instance_edit = changed_fields
                    .iter()
                    .any(|p| p.split('/').any(|seg| seg.contains("__")));
                ...
                if let Err(_e) = dep_graph.build_from_document(&nodes) {
                    ...
                    #[cfg(feature = "debug-resolver")]
                    println!(
                        "❌ Dependency tracking failed: {:?}, falling back to full resolution",
                        e
                    );
                    resolver::resolve_document(&mut nodes);
                } else {
                    ...
                    let original_changed_set: std::collections::HashSet<String> =
                        changed_fields.iter().cloned().collect();
                    let cascade_only_new: Vec<&String> = all_fields_to_update
                        .iter()
                        .filter(|f| !original_changed_set.contains(*f))
                        .collect();
                    let list_indicator_present = changed_fields
                        .iter()
                        .any(|p| p.split('/').any(|seg| seg == "L"));
                    ...
                    if cascade_only_new.is_empty() {
                        ...
                        for cf in &changed_fields {
                            if !cf.ends_with("/value") {
                                continue;
                            }
                            let base = cf.trim_end_matches("/value");
                            let parts: Vec<&str> =
                                base.split('/').filter(|seg| !seg.is_empty()).collect();
                            if parts.len() < 2 {
                                continue;
                            }
                            ...
                            let parent_parts: Vec<&str> = sibling_parent
                                .split('/')
                                .filter(|seg| !seg.is_empty())
                                .collect();
                            if let Some(parent_node) =
                                find_node_by_path_transparent(&nodes, &parent_parts)
                            {
                                for ch in &parent_node.children {
                                    if let Some(val) = ch.parameters.get("value") {
                                        if let OverseerValue::Formula(fstr) = val {
                                            ...
                                            {
                                                let sibling_path =
                                                    format!("{}/{}", sibling_parent, ch.name);
                                                let sibling_value_path =
                                                    format!("{}/value", sibling_path);
                                                if !all_fields_to_update
                                                    .contains(&sibling_value_path)
                                                {
                                                    all_fields_to_update
                                                        .insert(sibling_value_path.clone());
                    ...
                    if !all_fields_to_update.is_empty() {
                        ...
                        let mut templates_needed: std::collections::HashSet<String> =
                            std::collections::HashSet::new();
                        for cf in &changed_fields {
                            if !cf.contains("__") {
                                continue;
                            ...
                            let parts: Vec<&str> =
                                cf.split('/').filter(|s| !s.is_empty()).collect();
                            for seg in &parts {
                                if seg.contains("__") {
                                    if let Some(base) = seg.split("__").next() {
                                        if !base.is_empty() {
                                            templates_needed.insert(base.to_string());
                                        }
                                    }
                                }
                            }
                        ...
                        if !templates_needed.is_empty() {
                            ...
                            fn dfs_collect<'a>(
                                ...
                                for n in roots {
                                    ...
                                    if let Some(f) = dfs_collect(&n.children, name) {
                                        return Some(f);
                                    }
                                }
                                None
                            ...
                            let mut template_formula_paths: std::collections::HashMap<
                                String,
                                Vec<Vec<String>>,
                            > = std::collections::HashMap::new();
                            fn collect_formulas<'a>(
                                root: &'a OverseerNode,
                                prefix: &mut Vec<String>,
                                acc: &mut Vec<Vec<String>>,
                            ) {
                                if let Some(OverseerValue::Formula(_)) =
                                    root.parameters.get("value")
                                {
                            ...
                            fn find_instance_mut<'a>(
                                roots: &'a mut Vec<OverseerNode>,
                                path: &[&str],
                            ) -> Option<&'a mut OverseerNode> {
                                fn inner<'b>(
                                    nodes: *mut Vec<OverseerNode>,
                                    path: &[&str],
                                ) -> Option<*mut OverseerNode> {
                                    if path.is_empty() {
                                        return None;
                                    }
                                    unsafe {
                                        ...
                                        if path.len() == 1 {
                                            for n in vec_ref.iter_mut() {
                                                if n.name == path[0] {
                                                    return Some(n as *mut OverseerNode);
                                                }
                                            }
                                            return None;
                                        }
                                        for i in 0..vec_ref.len() {
                                            ...
                                            if vec_ref[i].is_hierarchy_transparent
                                                || vec_ref[i].name.is_empty()
                                            {
                                                if let Some(f) =
                                                    inner(&mut vec_ref[i].children, path)
                                                {
                                                    return Some(f);
                                                }
                                            }
                            ...
                            let snapshot_updates: Vec<String> =
                                all_fields_to_update.clone().into_iter().collect();
                            for u in snapshot_updates {
                                if !u.contains("__") || !u.ends_with("/value") {
                                    continue;
                                }
                                ...
                                let Some(rel_paths) = template_formula_paths.get(template_name)
                                else {
                                    continue;
                                };
                                ...
                                for rel in rel_paths {
                                    ...
                                    // Navigate to field
                                    if let Some(field_node) =
                                        find_instance_mut(&mut nodes, &abs_vec)
                                    {
                                        let has_formula = matches!(
                                            field_node.parameters.get("value"),
                                            Some(OverseerValue::Formula(_))
                                        );
                                        if !has_formula {
                                            if let Some(prev) =
                                                field_node.parameters.get("value").cloned()
                                            {
                                                field_node
                                                    .parameters
                                                    .insert("_computed_value".to_string(), prev);
                                            }
                    ...
                    if all_fields_to_update.is_empty() {
                        ...
                        #[cfg(feature = "debug-resolver")]
                        println!(
                            "🎯 Using selective resolution for {} fields",
                            all_fields_to_update.len()
                        );
                        ...
                        let original_set: std::collections::HashSet<String> =
                            changed_fields.iter().cloned().collect();
                        let expanded_only_new = all_fields_to_update
                            .iter()
