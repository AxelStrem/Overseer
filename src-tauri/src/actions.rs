...
impl ActionExecutor {
    ...
    pub fn next_due_ms(nodes: &Vec<OverseerNode>) -> Option<i64> {
        ...
        fn gather_template_def_paths<'a>(roots: &'a [OverseerNode]) -> Vec<Vec<String>> {
            ...
            fn scan_for_entries(n: &OverseerNode, names: &mut std::collections::HashSet<String>) {
                    ...
                    if let Some(v) = n.parameters.get("entry") {
                        match v {
                            OverseerValue::Template(t) => {
                                names.insert(t.trim().trim_matches(['<', '>']).to_string())
                            }
                ...
                for ch in &n.children {
                    scan_for_entries(ch, names);
                }
            }
            for r in roots {
                scan_for_entries(r, &mut names);
            }
            ...
            fn search_by_name<'a>(
                ...
                for ch in &cur.children {
                    search_by_name(acc, ch, path, roots, names);
                }
                if pushed {
                    path.pop();
                }
        ...
        fn is_under_any(prefixes: &Vec<Vec<String>>, path: &Vec<String>) -> bool {
            prefixes
                .iter()
                .any(|pre| path.len() >= pre.len() && &path[..pre.len()] == pre.as_slice())
        }
        ...
        fn collect<'a>(
            ...
            let pushed = if !cur.name.is_empty() {
                path.push(cur.name.clone());
                true
            } else {
                false
            ...
            for ch in &cur.children {
                collect(acc, ch, path, inside_instance, template_defs);
            }
            if pushed {
                path.pop();
            }
        ...
        for (t, tpath) in &timers {
            ...
            let active_val = t
                .parameters
                .get("_computed_active")
                .or_else(|| t.parameters.get("active"));
            ...
            let Some(at) = at_str.and_then(|s| Self::parse_timestamp_utc(&s)) else {
                continue;
            };
            // offset: prefer computed; evaluate formula if present; else parse
            let off_val = t
                .parameters
                .get("_computed_offset")
                .or_else(|| t.parameters.get("offset"));
            let off = match off_val {
                Some(OverseerValue::Formula(expr)) => {
                    let ctx = EvaluationContext::new(tpath.clone(), &snapshot);
                    FormulaEvaluator::evaluate_formula(expr, &ctx)
                        .ok()
                        .and_then(|v| Self::parse_offset_duration(&v))
                }
            ...
            let due_ms = if let Some(off) = off {
                ...
                if ms > 0 {
                    ...
                    if overdue {
                        debug_sched!(
                            "[SCHED] timer '{}' overdue (at+off: {:?} <= now: {:?}), due now",
                            t.name,
                            at + off,
                            now
                        );
                    ...
                    if at <= now {
                        debug_sched!("[SCHED] one-shot timer '{}' due now (at <= now)", t.name);
                        now.timestamp_millis()
                    } else {
                        at.timestamp_millis()
                    }
                ...
                if at <= now {
                    debug_sched!("[SCHED] one-shot timer '{}' due now (no offset)", t.name);
                    now.timestamp_millis()
                } else {
                    at.timestamp_millis()
                }
            ...
            #[cfg(feature = "debug-resolver")]
            {
                let off_dbg = match off_val {
                    Some(OverseerValue::String(s)) => Some(s.clone()),
                    Some(OverseerValue::Integer(i)) => Some(i.to_string()),
                    _ => None,
                };
                debug_sched!(
                    "[SCHED] scanned timer name='{}' active={} at={:?} offset={:?} next_due_ms={}",
                    t.name,
                    active,
                    at,
                    off_dbg,
        ...
        if timers_debug() {
            if let Some(_ms) = next_ms {
                debug_sched!(
                    "[SCHED] next_due_ms => {} ({} timers scanned)",
                    ms,
                    timers.len()
                );
            } else {
    ...
    fn run_timers(nodes: &mut Vec<OverseerNode>) -> Result<(), OverseerError> {
        ...
        fn collect(
            ...
            for child in &cur.children {
                collect(paths, child, path, inside_instance, template_defs);
            }
            if pushed {
                path.pop();
            }
        ...
        let template_def_paths = {
            ...
            for r in nodes.iter() {
                ...
                gather(r, &mut names, &mut dummy, &mut p);
            }
            ...
            fn collect_named(
                ...
                for ch in &cur.children {
                    collect_named(ch, path, names, acc);
                }
                path.pop();
            }
            ...
            for r in nodes.iter() {
                let mut p = Vec::new();
                collect_named(r, &mut p, &names, &mut acc);
            }
            acc
        };
        ...
        if timers_debug() {
            eprintln!("[TIMER] found {} timer path(s)", timer_paths.len());
        }
        ...
        'timers: for tpath in timer_paths {
            if actions_budget == 0 {
                if timers_debug() {
                    eprintln!("[TIMER] global actions budget exhausted; stopping scan");
                }
                break 'timers;
            }
            ...
            if let Some((ptr, _indices_initial)) = Self::get_node_mut_by_path(nodes, &tpath) {
                ...
                // Check active
                let active = match Self::get_effective(&timer_node.parameters, "active")
                    .or_else(|| timer_node.parameters.get("active"))
                {
                    Some(OverseerValue::Boolean(b)) => *b,
                    Some(OverseerValue::String(s)) => s.eq_ignore_ascii_case("true"),
                    _ => false,
                ...
                if !active {
                    if timers_debug() {
                        eprintln!("[TIMER] skip inactive '{}' at {:?}", timer_node.name, tpath);
                    }
                    continue;
                }
                if timers_debug() {
                    eprintln!(
                        "[TIMER] evaluating '{}' at path {:?}",
                        timer_node.name, tpath
                    );
                }
                // Evaluate 'at' (base timestamp)
                let at_val = Self::get_effective(&timer_node.parameters, "at")
                    .or_else(|| timer_node.parameters.get("at"));
                let at_str: Option<String> = match at_val {
                    Some(OverseerValue::Timestamp(ts)) => Some(ts.clone()),
                    Some(OverseerValue::Date(d)) => Some(format!("{}T00:00:00Z", d)),
                    Some(OverseerValue::String(s)) => Some(s.clone()),
                ...
                if let Some(at) = at_str {
                    ...
                    if let Some(base) = Self::parse_timestamp_utc(&at) {
                        let off_val = Self::get_effective(&timer_node.parameters, "offset")
                            .or_else(|| timer_node.parameters.get("offset"));
                        ...
                        if let Some(off) = offset {
                            ...
                            if off_ms > 0 {
                                ...
                                crate::formula_evaluator::FormulaEvaluator::set_time_override(
                                    Some(base),
                                );
                            }
                        } else if base <= now_dt {
                            ...
                            crate::formula_evaluator::FormulaEvaluator::set_time_override(Some(
                                base,
                            ));
                        }
                        ...
                        if fire_count > 0 {
                            ...
                            let dbg_path = {
                                let (_ptr, idxs) = match Self::get_node_mut_by_path(nodes, &tpath) {
                                    Some(v) => v,
                                    None => (std::ptr::null_mut(), vec![]),
                                };
                            ...
                            if timers_debug() {
                                eprintln!(
                                    "[TIMER] firing '{}' repeats={} (recurring={}) at path {}",
                                    timer_name, capped, is_recurring, dbg_path
                                );
                            }
                            ...
                            if !actions.is_empty() {
                                ...
                                let one_shot =
                                    match Self::get_effective(&timer_node.parameters, "one_shot")
                                        .or_else(|| timer_node.parameters.get("one_shot"))
                                    {
                                        Some(OverseerValue::Boolean(b)) => *b,
                                ...
                                if timers_debug() {
                                    eprintln!("[TIMER] will execute {} repeat(s) for '{}' at {} (budget left={})", repeats, timer_name, dbg_path, actions_budget);
                                }
                                ...
                                let exec_result = std::panic::catch_unwind(
                                    std::panic::AssertUnwindSafe(|| {
                                        'outer: for r in 0..repeats {
                                            ...
                                            let (owner_ptr, owner_indices) =
                                                match Self::get_node_mut_by_path(nodes, &tpath) {
                                                    Some(p) => p,
                                                    None => {
                                                        if timers_debug() {
                                                            eprintln!("[TIMER] owner path disappeared during actions: {:?}", tpath);
                                                        }
                                                        break 'outer;
                                                    }
                                                };
                                            let _ = owner_ptr; // don't keep pointer around
                                            let owner_path = Self::build_disambiguated_path(
                                                &nodes.clone(),
                                                &owner_indices,
                                            );
                                            for action in &actions {
                                                if actions_budget == 0 {
                                                    if timers_debug() {
                                                        eprintln!("[TIMER] actions budget exhausted (timer='{}' path={})", timer_name, dbg_path);
                                                    }
                                                    break 'outer;
                                                }
                                                if timers_debug() {
                                                    eprintln!("[ACTIONS] executing '{}' with params keys: {:?}", action.node_type, action.parameters.keys().collect::<Vec<_>>() );
                                                }
                                                let res = Self::execute_action(
                                                    nodes,
                                                    &owner_indices,
                                                    &owner_path,
                                                    action,
                                ...
                                match exec_result {
                                    ...
                                    Err(_panic_info) => {
                                        ...
                                        return Err(OverseerError::RuntimeError(format!(
                                            "Timer '{}' panicked during actions",
                                            timer_name
                                        )));
                                    }
                                }
                                ...
                                if one_shot {
                                    ...
                                    if did_any_action {
                                        if let Some((deact_ptr, _)) =
                                            Self::get_node_mut_by_path(nodes, &tpath)
                                        {
                                            let tn: &mut OverseerNode = unsafe { &mut *deact_ptr };
                                            match tn.parameters.get("active") {
                                                Some(OverseerValue::Formula(_)) => {
                                                    if timers_debug() {
                                                        eprintln!("[TIMER] one-shot '{}' at {} completed; leaving formula-based active intact", timer_name, dbg_path);
                                                    }
                                                }
                                                _ => {
                                                    tn.parameters.insert(
                                                        "active".to_string(),
                                                        OverseerValue::Boolean(false),
                                                    );
                                                    if timers_debug() {
                                                        eprintln!("[TIMER] deactivated one-shot timer '{}' at {}", timer_name, dbg_path);
                                                    }
                                                }
                                            }
                                        }
...
"#;
    ...
    fn test_timer_fires_and_deactivates() {
        ...
        let timer = root
            .children
            .iter()
            .find(|c| c.node_type == "timer")
            .unwrap();
        assert_eq!(
            timer.parameters.get("active"),
    ...
    fn test_timer_recurring_catch_up_fires_multiple_times_and_stays_active() {
        ...
        let root = nodes.iter().find(|n| n.name == "Root").unwrap();
        let a = root.children.iter().find(|c| c.name == "A").unwrap();
        let val = match a.parameters.get("value") {
            Some(OverseerValue::Integer(i)) => *i,
            _ => 0,
        };
        assert!(
            val >= 2 && val <= 5,
            "expected A between 2 and 5 inclusive, got {}",
            val
        );
        let timer = root
            .children
    ...
    fn test_recurring_timer_under_unnamed_wrapper_in_tab_appends_active() {
        ...
        assert!(
            ...
            "Active should have at least one appended item"
        );
        ...
        let next_id = state
            .children
            .iter()
            .find(|f| f.name == "next_id")
            .and_then(|f| f.parameters.get("value"))
            .cloned();
        // Depending on how many times it caught up, next_id should be >=2
        if let Some(OverseerValue::Integer(n)) = next_id {
            assert!(n >= 2 && n <= 6, "unexpected next_id {}", n);
        } else {
            panic!("next_id missing")
        }
        ...
        fn find_by_name<'a>(n: &'a OverseerNode, name: &str) -> Option<&'a OverseerNode> {
            ...
            for ch in &n.children {
                if let Some(r) = find_by_name(ch, name) {
                    return Some(r);
                }
            }
            None
        ...
        let recur = tasks
            .children
            .iter()
            .find_map(|c| find_by_name(c, "RecurringTask"))
            .expect("RecurringTask not found");
        let lta = recur
            .children
            .iter()
            .find(|f| f.name == "last_triggered_at")
            .and_then(|f| f.parameters.get("value"))
            .cloned();
