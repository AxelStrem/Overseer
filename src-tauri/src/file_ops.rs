...
use crate::types::*;
use crate::semantic_patch; // for emit_with_original adapter (available in both lib and bin crates)
...
impl OverseerFileHandler {
    ...
    // Temporary adapter: use semantic_patch::emit_with_original stub.
    pub fn emit_with_original(original: &str, nodes: &[OverseerNode]) -> Result<String> {
        let (text, _ops) = semantic_patch::emit_with_original(original, nodes)?;
        Ok(text)
    }
