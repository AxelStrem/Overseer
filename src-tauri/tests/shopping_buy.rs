...
//!
//! Pressing an entry copies it into History. Which shop it came from is not on the
...
/// Path to the entry whose `handle` field holds `handle` - the entry is what is pressed.
fn the_entry(nodes: &[OverseerNode], handle: &str) -> Vec<String> {
    ...
    for entry in &list.children {
        ...
        if stored.contains(handle) {
            return vec!["shopping".into(), "List".into(), entry.name.clone()];
        }
    }
...
fn buying_records_the_shop_that_was_being_looked_at() {
    ...
    let path = the_entry(&nodes, "bread");
