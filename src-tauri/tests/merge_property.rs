...
fn merge_preserves_comment_counts_over_examples() {
    ...
    fn collect(dir: &Path, out: &mut Vec<std::path::PathBuf>) {
        for e in fs::read_dir(dir).unwrap() {
            ...
            if p.is_dir() {
                collect(&p, out);
            } else if p.extension().map(|s| s == "os").unwrap_or(false) {
                out.push(p);
            }
        }
    }
    collect(&root, &mut files);
    assert!(!files.is_empty(), "no .os example files found");
    ...
    for file in files {
        ...
        let orig_comments: Vec<&str> = src
            ...
            .collect();
        let Ok((_rem, mut nodes)) = overseer::parser::parse_document(&src) else {
            continue;
        ...
        assert_eq!(
            orig_comments.len(),
            regen_comments.len(),
            "Comment line count changed for {:?}",
            file
        );
        let mut run = 0usize;
        let mut last = "";
        for c in &regen_comments {
            ...
            assert!(
                run <= 2,
                ">2 identical consecutive comment lines in {:?}",
                file
            );
        }
        checked += 1;
